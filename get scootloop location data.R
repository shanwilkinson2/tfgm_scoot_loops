...
# # all in one go - not working
# 
# alltogether <- httr::GET(
#   url = "https://api.tfgm.com/odata/ScootLoops?$expand=StartLocation,EndLocation&$top=10",
#   config = headers
#   ) %>%
# # parse as text
#   content("text") %>%
...
# 
#   
#   fromJSON(content(response, "text"))$value %>%
#   janitor::clean_names()
# # start & end points are each in thier little spatial data frame
